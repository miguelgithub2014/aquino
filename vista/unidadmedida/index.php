    ...
    <div class="row-fluid">
        <div>
            ...
            <option value="1">Prefijo</option>
                ...
                <a href="<?php echo BASE_URL ?>unidadmedida/nuevo" class="btn btn-grey"><i class="icon-plus icon-white"></i></a>
            </div>
            <br/>
        </div>
    </div>
    ...
    <table id="table" class="table table-striped table-bordered table-hover sortable">
        ...
        <?php for ($i = 0; $i < count($this->datos); $i++) { ?>
            <tr>
                ...
                <td><?php echo $this->datos[$i]['PREFIJO'] ?></td>
                <td><?php echo $this->datos[$i]['UNIDADBASE'] ?></td>
                <td><?php echo $this->datos[$i]['CANT_UNIDAD'] ?></td>
                <td>
                    <a href="javascript:void(0)" onclick="editar('<?php echo BASE_URL?>unidadmedida/editar/<?php echo $this->datos[$i]['ID_UNIDADMEDIDA'] ?>')" class="btn btn-success btn-minier"><i class="icon-pencil icon-white"></i></a>
                    <a href="javascript:void(0)" onclick="eliminar('<?php echo BASE_URL?>unidadmedida/eliminar/<?php echo $this->datos[$i]['ID_UNIDADMEDIDA'] ?>')" class="btn btn-danger btn-minier"><i class="icon-remove icon-white"></i></a>
                </td>
            </tr>
        <?php } ?>
        </tbody>
...
<div class="navbar-inner">
        <p>No hay unidadmedida</p>
        <a href="<?php echo BASE_URL?>unidadmedida/nuevo" class="btn btn-primary">Nuevo</a>
    <?php } ?>