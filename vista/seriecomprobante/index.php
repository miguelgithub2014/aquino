    ...
    <div class="row-fluid">
        <div>
            ...
            <select class="m-wrap list" id="filtro">
                ...
                <input class="m-wrap input-xlarge" type="text" id="buscar" placeholder="Buscar Comprobante">    
                <button class="btn btn-inverse" type="button" id="btn_buscar"><i class="icon-search icon-white"></i></button>
                <a href="<?php echo BASE_URL ?>seriecomprobante/nuevo" class="btn btn-grey"><i class="icon-plus icon-white"></i></a>
            </div>
            <br/>
        </div>
    </div>
    ...
    <table id="table" class="table table-striped table-bordered table-hover sortable">
        ...
        <?php for ($i = 0; $i < count($this->datos); $i++) { ?>
            <tr>
                ...
                <td><?php echo $this->datos[$i]['MAXCORRELATIVO'] ?></td>
                <td>
                    <a href="javascript:void(0)" onclick="editar('<?php echo BASE_URL?>seriecomprobante/editar/<?php echo $this->datos[$i]['ID_SERIECOMPROBANTE'] ?>')" class="btn btn-success btn-minier"><i class="icon-pencil icon-white"></i></a>
                    <a href="javascript:void(0)" onclick="eliminar('<?php echo BASE_URL?>seriecomprobante/eliminar/<?php echo $this->datos[$i]['ID_SERIECOMPROBANTE'] ?>')" class="btn btn-danger btn-minier"><i class="icon-remove icon-white"></i></a>
                </td>
            </tr>
        <?php } ?>
        </tbody>
...
<div class="navbar-inner text-center">
        <br/>
        <p>No hay seriecomprobante</p>
        <a href="<?php echo BASE_URL?>seriecomprobante/nuevo" class="btn btn-primary">Nuevo</a>
    <?php } ?>