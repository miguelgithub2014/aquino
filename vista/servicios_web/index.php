...
<?php if (isset($this->datos) && count($this->datos)) { ?>
    <div class="row-fluid">
        <div>
            ...
            <option value="0">Titulo</option>
                ...
                <a href="<?php echo BASE_URL ?>servicios_web/nuevo" class="btn btn-grey"><i class="icon-plus icon-white"></i> Nuevo</a>
            </div>
            <br/>
        </div>
    </div>
    ...
    <table id="table" class="table table-striped table-bordered table-hover sortable">
        ...
        <?php for ($i = 0; $i < count($this->datos); $i++) { ?>
            <tr>
                ...
                <td>
                    <a href="#myModal" role="button" data-toggle="modal" onclick="ver('<?php echo $this->datos[$i]['ID'] ?>')" class="btn btn-warning btn-minier"><i class="icon-eye-open icon-white"></i> Ver</a>
                    <a href="javascript:void(0)" onclick="editar('<?php echo BASE_URL?>servicios_web/editar/<?php echo $this->datos[$i]['ID'] ?>')" class="btn btn-success btn-minier"><i class="icon-pencil icon-white"></i> Editar</a>
                    <a href="javascript:void(0)" onclick="eliminar('<?php echo BASE_URL?>servicios_web/eliminar/<?php echo $this->datos[$i]['ID'] ?>')" class="btn btn-danger btn-minier"><i class="icon-remove icon-white"></i> Eliminar</a>
                </td>
            </tr>
        <?php } ?>
        </tbody>
    ...
    <div class="page-header">
        <p>No hay Servicios de la web</p>
        <a href="<?php echo BASE_URL?>servicios_web/nuevo" class="btn btn-primary">Nuevo</a>
    </div>
<?php } ?>
